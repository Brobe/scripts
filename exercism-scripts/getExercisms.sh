...
for e in "${exers[@]}"; do
    ...
    if [[ -d "$HOME/exercism/$prog/$e" ]]; then
        ((curr++))
        ~/scripts/progressbars/progress-bar -p "$curr $count"
        continue
    fi
    while true; do
        ...
        if echo $exeroutput | grep -q "Error"; then
            if echo $exeroutput | grep -q "status 429"; then
                ~/scripts/progressbars/progress-bar -e "Too many requests, taking a break ($curr of $count done)"
                for ((i = 40; i > 0; i--)); do
                    echo -ne "\r$i "
                    sleep 1
                done
                ...
                ~/scripts/progressbars/progress-bar -e "exercise $e already exists"
            else
                ~/scripts/progressbars/progress-bar -e "unknown error $exeroutput"
            fi
        fi
        break
    done
...
# ~/scripts/progressbars/progress-bar -p "$count $count"
